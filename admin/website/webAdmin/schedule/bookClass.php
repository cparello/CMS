...
function bookSessionMemberClass() {
      ...
      $dbMain = $this->dbconnect();
      $stmt = $dbMain ->prepare("SELECT class_count FROM schedular_member_class_count WHERE sm_contract_key = '$this->smContractKey' AND service_key='$this->serviceKey'");
      $stmt->execute();      
      $stmt->store_result();      
...
function loadMemberClassStatus() {
 ...
 $stmt = $dbMain ->prepare("SELECT DISTINCT group_type, group_number, service_key, service_quantity, service_term FROM paid_full_services WHERE contract_key = '$this->contractKey' AND service_term != 'C'");
 $stmt->execute();      
 $stmt->store_result();      
 $stmt->bind_result($group_type, $group_number, $service_key, $service_quantity, $service_term);
 $pifRowCount = $stmt->num_rows;
 ...
 if($pifRowCount > 0) {
    ...
    while ($stmt->fetch()) {
              ...
              $this->duration = $service_quantity;
...
if($this->bookingStatus == null)  {
    ...
    $stmt = $dbMain ->prepare("SELECT DISTINCT group_type, group_number, service_key, number_months FROM monthly_services WHERE contract_key ='$this->contractKey'  ORDER BY signup_date DESC");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($group_type, $group_number, $service_key, $number_months);
