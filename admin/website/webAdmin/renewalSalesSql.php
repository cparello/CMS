...
$nowSecs = strtotime($dateNow);
if ($endSecs > $nowSecs){
       // echo "$end_date  $endSecs  $nowSecs";
...
$this->terms = "$contract_terms<br><br>Liability Term: $liability_terms";
...
$pifProcFeeBool = 0;
    ...
    $sArray = explode(',',$sale);
    $sNameBuff = explode('-',$saleArray[0]);
    
    $this->serviceNamePurchased = trim($sNameBuff[0]);
    $this->clubName = trim($sNameBuff[1]);
    $this->numberOfServicePourchased = trim($saleArray[1]);
    $this->serviceCost = trim($saleArray[2]);
    $this->serviceQuantityPurchased = trim($saleArray[3]);
    
    if (!preg_match('/All/',$this->clubName)){
    ...
    $stmt = $dbMain ->prepare("SELECT service_key FROM service_info WHERE service_type = '$this->serviceNamePurchased' AND club_id = '$this->clubId'");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($this->serviceKey);   
    $stmt->fetch();   
    $stmt->close();
    
    $stmt = $dbMain ->prepare("SELECT service_term FROM service_cost WHERE service_key = '$this->serviceKey' AND service_quantity = '$this->serviceQuantityPurchased'");
    $stmt->execute();      
    $stmt->store_result();      
   ...
   // echo "aa $this->serviceKey bb $this->serviceTerm cc $this->serviceNamePurchased dd $this->clubId";
  //  exit;
    
        $this->transfer = "N";
    
    
    $this->todaysPayment = $this->serviceCost;
