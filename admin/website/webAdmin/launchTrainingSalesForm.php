...
foreach($newServiceListArray as $service){
    ...
    if($numberMembershipsArray[$counter] > 0){ 
        if ($newServiceQuantityArray[$counter] == 1){
            $newServiceQuantityArray[$counter] = "1 Class";
            $classText = "";
        }else{
            $classText = "Classes";
        }
        $this->summaryHtml .= "
            ...
            <td colspan=\"3\" class=\"sumtext\">
            <bb style=\"padding-left: 10px;\"><b>$newServiceQuantityArray[$counter] $classText</b>
            </td>
            </tr>";
       ...
       $this->totalCost += $newServiceCostArray[$counter];
          for($i=1;$i<=$numberMembershipsArray[$counter];$i++){
            ...
            //$testKey = $_SESSION['userContractKey'];
           // echo "$testKey";
            //exit;
...
<div class=\"product-image\">
 ...
 if(!isset($_SESSION['userContractKey'])){
                    $alreadyMember = 0;
                    $memberHtml = "<div id=\"membersInfoTab\">
...
include"webTemplates/trainingSalesFormTemplate.php";
