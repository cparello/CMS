	...
	function loadClassOptionsTwo(scheduleType, clubId, className, barCodeType, groupType, firstName, lastName, phone, email) {
		...
		$.ajax({
			...
			// alert(data);
				...
				var listings = dataArray[1];
				if (successBit == 1) {
					$('#msgBox2').html(listings);
					$("#sm_fname").val(firstName);
					$("#sm_lname").val(lastName);
					$("#sm_email").val(email);
					$("#sm_phone").val(phone);
				} else if (successBit == 2) {
				    $('#msgBox2').html('A bundle class does not exist for this service. Please set this up in the administration interface in order to display class pricing');
                        $("#msgBox2").css( { "color" : "red"} );
				} else {
					alert(data);
				}
