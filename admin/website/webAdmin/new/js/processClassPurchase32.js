...
//---------------------------------------------------------
    function saveScheduleSale(memberId, serviceKey, classNumber, serviceCost, locationId, contractKey) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (purchaseBit == 1) {
                    ...
                    $('#msgBox5').html('There was an error processing this request!<br>' + data + ' ');
...
//---------------------------------------------------------
    function saveSchedulerMember(memberId, serviceKey, classNumber, firstName, lastName, email, phone, locationId, contractKey) {
...
//---------------------------------------------------------
    function checkPhoneNumber(phoneValue) {
        ...
        if (regexObj.test(phoneValue)) {
            ...
            error = 'You have entered an invalid Phone Number or format.  The Phone Number must contain an area code followed by the number<br>';
            return error;
        }
...
//===================================================
    $('#purchase_class').click(function () {
        ...
        var classDate = $('#class_date').val();//$(".datepicker").val();
        var classText = $("#class_text").val();
        var typeId = $("#type_id").val();
        var locationId = $("#location").val();
        var className = $("#class_name").val();
        var timeSlot = $("#time_slot").val();
        var bookingCount = $(this).closest('td').find('#booking_count').text();//$('#booking_count').val();
        var groupType = "";
        var groupTypeTwo = "";
        var ajaxSwitch = 1;
        var errors = "";
        ...
        if ($('input:radio[name=sessions]:checked').val() == undefined) {
        ...
        if (document.getElementById('terms_conditions').checked == false) {
            errors = errors + "Please agree to the class waiver.<br>";
        }
...
//set vars for  confirmation
        ...
        if (memberId == "" && document.getElementById('non_member').checked == false) {
            errors = errors + "Member Id field cannot be empty<br>";
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (paymentStatus == 1) {
                        ...
                        saveSchedulerMember(memberId, serviceKey, classNumber, firstName, lastName, email, phone, locationId, contractKey);
                        saveScheduleSale(memberId, serviceKey, classNumber, serviceCostText, locationId, contractKey);

                        if (newNonMemBool == 1) {
                            var xtraMsg = ' A email with a barcode has to been sent to the email you entered.'
                            $('#bookClass').prop("disabled", false);
                        } else {
                            var xtraMsg = '';
                        }
                        //alert('Payment of '+serviceCostText+' successfully processed'); 
                        $('#msgBox5').html('Payment of ' + serviceCostText + ' successfully processed. <br>' + xtraMsg);
                        $("#msgBox5").css({"color": "green"});
                    } else {
                        $('#purchase_class').prop("disabled", false);
                        // $('#purchase_class').attr("class", "button1");
                        $('#msgBox5').html('There was an error processing this transaction ' + data + '<br>Please recheck your CC info or use a different card.');
                        $("#msgBox5").css({"color": "red"});
                        return false;
                    }
