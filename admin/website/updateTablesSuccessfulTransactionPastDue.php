...
//$this->cycleEndDay = $this->cycleStartDay + $this->pastDays;
    if ($this->billingAmount > $this->lesserValue) { 
            ...
            $sql = "UPDATE billing_scheduled_recuring_payments SET billing_amount = ?  WHERE contract_key = '$this->contractKey' AND payment_id = '$payment_id'";
            $stmt = $dbMain->prepare($sql);
            $stmt->bind_param('d', $stillOwed);
            if(!$stmt->execute())  {
