...
function readWriteData(){
        ...
        $dbMain = $this->dbconnect();
    $stmt = $dbMain->prepare("SELECT bank_name, account_type, account_number, routing_number  FROM banking_info WHERE contract_key = '$this->contractKey'");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($this->bankName, $this->bankAccountType, $this->eftCheckingAccountNumber, $this->routingNumber);   
    $stmt->fetch();   
    $stmt->close();
    
    $stmt = $dbMain->prepare("SELECT card_type, card_number, card_exp_date FROM credit_info WHERE contract_key = '$this->contractKey'");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($this->CardCreditCardType, $this->CardCreditCardNumber, $this->CardCreditCardExpirationDate);   
    $stmt->fetch();   
    $stmt->close();
    ...
    $stmt = $dbMain->prepare("SELECT first_name, middle_name, last_name, street, city, state, zip, primary_phone, cell_phone, email FROM contract_info WHERE contract_key = '$this->contractKey'");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($this->contractFirstName,$this->contractAccountMname,$this->contractLastName,$this->contractStreet,$this->contractCity,$this->contractState,$this->contractZipCode,$this->contractPrimaryPhone,$this->contractCellPhone,$this->contractEmailAddress);   
    $stmt->fetch();   
    $stmt->close();
    
    $stmt = $dbMain->prepare("SELECT monthly_billing_type, billing_amount FROM monthly_payments WHERE contract_key = '$this->contractKey'");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($this->monthlyBillingType,$this->nextBillingFee);   
    $stmt->fetch();   
         

        $this->outFile .= "$this->contractKey,$this->memberId,$this->contractFirstName,$this->contractAccountMname,$this->contractLastName,$this->contractStreet,$this->contractCity,$this->contractState,$this->contractZipCode,$this->contractPrimaryPhone,$this->contractCellPhone,$this->contractEmailAddress,$this->bankName,$this->bankAccountType,$this->eftCheckingAccountNumber,$this->routingNumber,$this->CardCreditCardType,$this->CardCreditCardNumber,$this->CardCreditCardExpirationDate,$this->monthlyBillingType,$this->dueDate,$this->billingAmount \r\n";
...
while($stmt999->fetch()){
    //echo "test $counter $this->contractKey<br>";
                     ...
                     //   echo " $this->prePayCount ";
                      ...
                      //  echo " $this->monthlySettledCount $this->dueDate$dateFlag<br>";                                                                                                
                                if ($this->monthlySettledCount == 0){
                                    ...
                                    $this->dueDate = date('Y-m-d H:i:s',mktime(23,59,59,2,10,2015));
        ...
        $fileHeader = "contractKey,barcode,contractFirstName,contractAccountMname, contractLastName, contractStreet, contractCity, contractState, contractZipCode, contractPrimaryPhone, contractCellPhone, contractEmailAddress, bankName, bankAccountType,  eftCheckingAccountNumber, routingNumber, CardCreditCardType, CardCreditCardNumber, CardCreditCardExpirationDate,  monthlyBillingType, nextBillingDate, billingAmount \r\n";
