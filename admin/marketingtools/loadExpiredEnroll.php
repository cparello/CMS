...
$fileName = "EexpiredRenew$this->dateRange.pdf";
$invoiceSalt = rand(1000, 9000);
$tempFile = "testFile$invoiceSalt.html";
$contentFile = "/var/www/vhosts/ems/$domainDir/admin/expired/$tempFile";

file_put_contents($contentFile, $this->printableInvoice);

exec("/usr/local/bin/wkhtmltopdf  -s Letter --outline -T 0 -B 0 -R 0 -L 0 $contentFile /var/www/vhosts/ems/$domainDir/admin/expired/$fileName");

unlink("$contentFile");

//rename the file so it is more legible for download
$fileDate = date("m_d_Y", $this->dateStart);
$newFileName = "Expired_Re_Enroll_$this->dateRange.pdf";

rename("/var/www/vhosts/ems/$domainDir/admin/expired/$fileName", "/var/www/vhosts/ems/$domainDir/admin/expired/$newFileName");
...
//--------------------------------------------------------------------------------------------------------------------
function loadExpiredEnrollParameters() {
...
$this->erTextOne = $er_txt_one;
$this->erTextTwo = $er_txt_two;

$this->invoiceHeader = 'SERVICE EXPIRED';
$stmt->close();  
...
$this->todaysDate = date("M j, Y");
  ...
  while ($stmt->fetch()) {        
            ...
            $stmt99 = $dbMain ->prepare("SELECT num_text_primary, num_calls_primary, num_text_cell, num_calls_cell, num_emails FROM account_phone_spam_check WHERE contract_key = '$contract_key' AND report_type = 'EE' AND month = '$this->month' AND year = '$this->year'");
            $stmt99->execute();      
            $stmt99->store_result();                       
...
Your current $this->serviceName has expired.
...
if($this->listType == "phone") {
  $titleDiv = "
  <div id=\"userHeader\">
   Expired Re-enrollment Phone List 
  </div>";
...
if($this->listType == "email") {
  $titleDiv = "
  <div id=\"userHeader\">
   Expired Re-enrollment Email List 
  </div>";
  $cssFIle = 'phoneEmail.css';
  $tableHeadTag = "<table align=\"left\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\" width=100%>\n";
  $tabHead = "
...
<title>Expired Re-enrollment $this->dateRange Days</title>
...
<title>Early Renewable $this->currentStatementDate</title>
