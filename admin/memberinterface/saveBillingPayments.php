...
unset($_SESSION['cc_request_id']);
    ...
    $flag2 = "Y";
   $flag = "N";
   $sql = "UPDATE batch_recurring_records SET processed = ?, imported = ?, outstanding_balance= ? WHERE contract_key = '$this->contractKey'"; // AND history_key= '$this->transKey' 
   $stmt = $dbMain->prepare($sql);
   $stmt->bind_param('sss', $flag2, $flag2, $flag); 
   $rejectBit = 1;
...
$stmt = $dbMain ->prepare("SELECT count(*) as count, next_payment_due_date FROM monthly_settled WHERE contract_key = '$contractKey'");
$stmt->execute();      
$stmt->store_result();      
...
$billCycStart = date('Y-m-d H:i:s',mktime(23,59,59,date('m',strtotime($next_payment_due_date)),date('d',strtotime($next_payment_due_date))-$this->pastDay,date('Y',strtotime($next_payment_due_date))));
...
if ($count == 0){
                    ...
                    if ($curD <= $this->cycleDay  AND $curM != $mStart){
                        $nextPaymentDueDate = date("Y-m-d H:i:s"  , mktime(23,59,59,$curM,$day+$this->pastDays,$curY));
                    }else if ($curD <= $this->cycleDay AND $curM == $mStart){
                        $nextPaymentDueDate = date("Y-m-d H:i:s"  , mktime(23,59,59,$curM+1,$day+$this->pastDays,$curY));
                    }else if ($curD > $this->cycleDay){//or not = AND $curM == $mStart
                        $nextPaymentDueDate = date("Y-m-d H:i:s"  , mktime(23,59,59,$curM+1,$day+$this->pastDays,$curY));
                         }
