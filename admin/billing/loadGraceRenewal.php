...
array_map('unlink', glob("/var/www/vhosts/ems/$domainDir/admin/gracerenewal/*.pdf"));

$fileName = "GracePeriodRenew$this->dateEnd.pdf";
$invoiceSalt = rand(1000, 9000);
$tempFile = "testFile$invoiceSalt.html";
$contentFile = "/var/www/vhosts/ems/$domainDir/admin/gracerenewal/$tempFile";

file_put_contents($contentFile, $this->printableInvoice);

exec("/usr/local/bin/wkhtmltopdf  -s Letter --outline -T 0 -B 0 -R 0 -L 0 $contentFile /var/www/vhosts/ems/$domainDir/admin/gracerenewal/$fileName");

unlink("$contentFile");

//rename the file so it is more legible for download
$fileDate = date("m_d_Y", $this->dateEnd);
$newFileName = "Grace_Period_Renew_$fileDate.pdf";

rename("/var/www/vhosts/ems/$domainDir/admin/gracerenewal/$fileName", "/var/www/vhosts/ems/$domainDir/admin/gracerenewal/$newFileName");
...
//--------------------------------------------------------------------------------------------------------------------
function loadGraceParameters() {

$dbMain = $this->dbconnect();
$stmt = $dbMain ->prepare("SELECT grace_header, grace_txt FROM renewal_invoice_options WHERE invoice_key = '1'");
$stmt->execute();      
$stmt->store_result();   
$stmt->bind_result($grace_header, $grace_txt);
$stmt->fetch();

$this->graceHeader = $grace_header;
$this->graceText = $grace_txt;
$this->invoiceHeader = 'GRACE PERIOD RENEWAL';
$stmt->close();  
...
function loadRecordCount() {
  ...
  //$this->expDate = date("M j, Y" ,$this->dateEnd);
  
  $standardSecondsGrace = 86400 * $this->standardRenewalGrace;
  $statementRangeEndDateSecs = $standardSecondsGrace + $this->dateEnd;  
  $this->statementRangeEndDate = date("M j, Y" ,$statementRangeEndDateSecs);
  ...
  while ($stmt->fetch()) { 
            ...
            $stmt99 = $dbMain ->prepare("SELECT num_text_primary, num_calls_primary, num_text_cell, num_calls_cell, num_emails FROM account_phone_spam_check WHERE contract_key = '$contract_key' AND report_type = 'GP' AND month = '$month' AND year = '$year'");
            $stmt99->execute();      
            $stmt99->store_result();                       
  ...
  $reportType = 'GP';
    ...
    if(!$stmt->execute())  {
    	printf("Error: %s.Insert GP renewable\n", $stmt->error);
       }		
    $stmt->close();  
...
mail("$this->emailAddress", "$this->graceHeader",

"Hello, $this->firstName $this->middleName $this->lastName

You are eligible to renew the contract for your current $this->serviceName service within $this->standardRenewalGrace day(s) of it's
expiration at your current contract rate.
...
<span class="left">
Statement Period:  &nbsp; $this->todaysDate  
</span>
</td>
...
if($this->listType == "phone") {
  $titleDiv = "
  <div id=\"userHeader\">
   Grace Period Renewal Phone List 
  </div>";
...
if($this->listType == "email") {
  $titleDiv = "
  <div id=\"userHeader\">
   Grace Period Renewal Email List 
  </div>";
  $cssFIle = 'phoneEmail.css';
  $tableHeadTag = "<table align=\"left\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\" width=100%>\n";
  $tabHead = "
...
<title>Grace Period $this->todaysDate</title>
...
<title>Grace Period Renewable $this->todaysDate</title>
