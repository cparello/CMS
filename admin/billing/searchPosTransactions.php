...
$stmt = $dbMain ->prepare("SELECT  COUNT(category_name) AS cat_count  FROM merchant_sales WHERE category_name LIKE  '%$this->category%'");   
$stmt->execute(); 
$stmt->store_result();      
...
$dbMain = $this->dbconnect();
$stmt = $dbMain ->prepare("SELECT  COUNT(bar_code) AS barcode_count  FROM merchant_sales WHERE bar_code = '$this->barcode'");   
$stmt->execute();
$stmt->store_result();      
...
$dbMain = $this->dbconnect();
$stmt = $dbMain ->prepare("SELECT COUNT(purchase_date) AS date_count  FROM merchant_sales WHERE (purchase_date BETWEEN '$start' AND '$end')");   
$stmt->execute(); 
$stmt->store_result();      
...
$_SESSION['account_search_sql'] = "(purchase_date BETWEEN '$start' AND '$end')";
