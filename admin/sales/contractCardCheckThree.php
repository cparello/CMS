...
//echo"M: $ccCardMonth  Y: $ccCardYear";
//exit;
...
if($achBool != "" &&  $ccBool != "") {
    ...
    $merchField1 = "ACH Renewal Sales";
    $gw = new gwapi;
    $gw->setLogin("$userName", "$password");
    $gw->setBilling("$achFirstName","$achLastName","","$achStreetAddress","", "$achCity",
	...
	$check = null;
      ...
      $name = "$ccFirstName $ccLastName";
       $amount = $ccCardPayment;
        ...
        unset($_SESSION['track1']);
        unset($_SESSION['track2']);
        //$dupSecs = 3000;
            //======================== 
            
        $gw = new gwapi;
        $gw->setLogin("$userName", "$password");
        $gw->setBilling("$ccFirstName","$ccLastName","","$achStreetAddress","", "$achCity",
        "$achState","$achZip","US","$achPhone","$achPhone","$achEmail",
        "");
        $r = $gw->doSale($amount, $ccCardNumber, $ccexp, $cvv, $payTypeFlag, $orderId, $merchField1, $track1, $track2, $ccFirstName, $ccLastName);
...
if($achAuthReasonCode != 100) {
    $_SESSION['ach_request_id'] = $transactionId;
    $ach_result = "There was a problem verifying this Banking information.  Please check the Banking fields and re-enter this information. ERROR: $achAuthReasonCode";
    echo"$ach_result";
    exit;   
   }else{   
    $card_result = 1;             
