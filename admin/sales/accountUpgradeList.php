...
function loadColorCodes() {
 ...
 //  echo"$end_grace_date";
   ...
   $service_end_date  = strtotime($service_end_date);
    ...
    if($this->serviceEndDate != '0000-00-00') {
 ...
 switch($this->serviceStatusBilling) {          
             ...
             $this->expiredDisabled = 'disabled="disabled"';
...
if($credit_bit == 1) {
  ...
  $option_values ="<option value=\"C\" selected>Class</option>";
}
...
$billing_listing ="
 <tr id=\"b$this->billingFieldCount\" bgcolor=\"$this->backgroundColor\" class=\"whiteBg\">
...
$stmt = $dbMain ->prepare("SELECT DISTINCT service_key, service_name FROM monthly_services WHERE contract_key ='$this->contractKey'");
$stmt->execute();      
$stmt->store_result();   
$stmt->bind_result($service_key, $service_name);
$rowCount = $stmt->num_rows;
$this->serviceCount = $this->serviceCount + $rowCount;
             ...
             if($rowCount == 0) {
                 $this->monthlyServices = null;                
                    ...
                    while ($stmt->fetch()) {
                             $this->serviceKey = $service_key;     
                             $cancelValue = $this->checkAccountStatusFull();
                             if(preg_match("/membership/i", $service_name) AND $cancelValue != 'CA') {
                                  $this->upgradeFlag = 1;
                                  $this->upgradeServiceKey = $service_key;
                                  }else{
                                    $this->upgradeFlag = 0;
                                    $this->upgradeServiceKey = 0;
                                  }   
...
function getUpgradeFlag() {
             return($this->upgradeFlag);
             }             
function getUpgradeServiceKey() {
             return($this->upgradeServiceKey);
             }              
