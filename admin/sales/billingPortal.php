...
if($achBool != "" &&  $ccBool != "") {
    ...
    $request->merchantReferenceCode = "CC Upgrade Sales";
...
if($ccAuthReasonCode != 100) {
  ...
  exit;
    ...
    $request = new stdClass();
    $request->merchantID = MERCHANT_ID;
    $request->merchantReferenceCode = "ACH Upgrade Sales";
    $request->clientLibrary = "PHP";
    $request->clientLibraryVersion = phpversion();
    $request->clientEnvironment = php_uname();  
    ...
    $gw->setBilling("$achFirstName","$achLastName","","$achStreetAddress","", "$achState",
        ...
        "");
    $r = $gw->doAchSale($payTypeFlag, $checkName, $achRoutingNumber, $achAccountNumber, $account_holder_type, $account_type, $merchField1);
