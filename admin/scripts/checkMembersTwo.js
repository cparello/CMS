...
var memIdLength = memberId.length;
var primaryContact = ('Member Information ' +addSalt);
 ...
 if(memberId == "0") {
           alert('The \"' +primaryContact+ '\" Member ID field cannot be set to zero');
           document.getElementById('member_id' +addSalt).focus();
           return false;
           }           
if(isNaN(memberId)) {
           alert('The \"' +primaryContact+ '\" Member ID field may only contain numbers');
           document.getElementById('member_id' +addSalt).focus();
           return false;
           }
if(memIdLength < 4) {
           alert('The \"' +primaryContact+ '\" Member ID number is too short');
           document.getElementById('member_id' +addSalt).focus();
           return false;
           }
...
window.open("takeSnapshot.php" +parameters+ "","","scrollbars=yes,menubar=no,height=455,width=525,resizable=no,toolbar=no,location=no,status=no");
...
//----------------------------------------------------------------------------------------------------
function saveContactRecord(addSalt, contactId, fieldName, fieldId, contractKey)  {
...
xmlHttp.open("POST", "../utilities/updateMemberInfoTwo.php", true);
