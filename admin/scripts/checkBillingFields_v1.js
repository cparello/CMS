...
var row;
var rowId = 1;
var servName = 'subtract'
var serveNameId;
var fieldBit = "";

        for(var j = 2, row; row = serviceTable.rows[j]; j++) {        
                   serveNameId = (servName + rowId);            
                if(document.getElementById(serveNameId).checked == true) {
                   fieldBit = 1;
                  }
                rowId++;
            }
             ...
             if(creditTrue != "")  {
                if(fieldBit != 1) {
                   var response =  confirm('This will extend the Service Term of selected service(s). Do you wish to continue?');
                     ...
                     var response =  confirm('This will shorten the Service Term of selected service(s). Do you wish to continue?');
                             if(!response) {   
                              document.getElementById("upgrade_service_bit").value = "";
                               return false;
                              }else{
                              document.getElementById("upgrade_service_bit").value = 2;
                              }
                 }
...
if(pastDueFieldValue != '0.00') {
 var skipIt = 0;
      if(creditFieldValue == "" && achFieldValue == "" &&  cashFieldValue == "" &&  checkFieldValue == "") {
         var response =  confirm('You did not enter a Past Due Value. Do you wish to continue?');
                             if(!response) {   
