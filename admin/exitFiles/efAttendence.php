...
//==============================================================================================
function fileMaker(){
   // $ourFileName = "exitFileAtendence.csv";
            //$ourFileHandle = fopen($ourFileName, 'a');

            $this->file = "\"location_id\",\"member_id\",\"attendance_date\",\"access_flag\",\"attendance_type\",\"check_in_type\"\n";
    ...
    $stmt = $dbMain->prepare("SELECT location_id, member_id, attendance_date, access_flag, attendance_type, check_in_type FROM attendance_records WHERE member_id != ''");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($location_id, $member_id, $attendance_date, $access_flag, $attendance_type, $check_in_type);   
    while($stmt->fetch()){
             echo "$member_id <br>";
