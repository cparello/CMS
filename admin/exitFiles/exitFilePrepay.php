...
function readData(){
    ...
    $stmt = $dbMain->prepare("SELECT num_months, payment_amount, service_keys, payment_date, restart_date, restart_payment FROM pre_payments WHERE contract_key = '$this->contractKey'");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($this->num_months, $this->payment_amount, $this->service_keys, $this->payment_date, $this->restart_date, $this->restart_payment);   
    $stmt->fetch();
    $stmt->close();
    
    $ourFileName = "exitFilePrepay.csv";
    $ourFileHandle = fopen($ourFileName, 'a');
    
    $this->file = "\"$this->contractKey\",\"$this->num_months\",\" $this->payment_amount\",\" $this->service_keys\",\" $this->payment_date\",\" $this->restart_date\",\" $this->restart_payment\" \n";
...
function fileMaker(){
    ...
    $this->file = "\"contract key\",\"num_months\",\" payment_amount\",\" service_keys\",\" payment_date\",\" restart_date\",\" restart_payment\"\n";
    ...
    $dbMain = $this->dbconnect();
    $stmt = $dbMain ->prepare("SELECT DISTINCT contract_key FROM pre_payments WHERE contract_key != '0'");
    $stmt->execute();      
    $stmt->store_result();      
