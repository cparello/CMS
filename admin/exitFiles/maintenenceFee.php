...
function fileMaker(){
    ...
    $ourFileName = "exitFileMaintFee.csv";
    $ourFileHandle = fopen($ourFileName, 'a');
    
    $this->file = "  contract_key, m_cycle, m_cycle_date, m_fee\n";
    ...
    $stmt = $dbMain ->prepare("SELECT contract_key, m_cycle, m_cycle_date, m_fee FROM member_maintnence_eft WHERE contract_key != '0'");
    $stmt->execute();      
    $stmt->store_result();      
    $stmt->bind_result($contract_key, $m_cycle, $m_cycle_date, $m_fee);
    while ($stmt->fetch()) { 
                echo"$type_key  <br> ";
                $ourFileName = "exitFileMaintFee.csv";
                $ourFileHandle = fopen($ourFileName, 'a');
