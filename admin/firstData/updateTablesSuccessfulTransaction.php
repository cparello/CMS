...
$stmt999 = $dbMain->prepare("SELECT payment_id, contract_key, payment_type, transaction_type, authorization_id, attempt_number, billing_amount, cycle_start_day, cycle_start_month, cycle_start_day, cycle_start_year FROM batch_recurring_records WHERE processed = 'Y' AND imported = 'N' AND outstanding_balance = 'N'");
             if(!$stmt999->execute())  {
...
//$this->cycleEndDay = $this->cycleStartDay + $this->pastDays;
 ...
 $this->insertPaymentHistory();
  ...
  $imported = 'Y';
  $sql = "UPDATE batch_recurring_records SET imported = ? WHERE contract_key = '$this->contractKey' AND payment_id = '$payment_id'";
  $stmt = $dbMain->prepare($sql);
