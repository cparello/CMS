...
for($i=1;$i<=12;$i++){
   ...
   $stmt = $dbMain ->prepare("SELECT count(*), SUM(payment_amount) FROM payment_history WHERE payment_flag ='PF' AND contract_key='$this->contractKey' AND MONTH (payment_date) = '$i' AND YEAR (payment_date) = '2015' AND (payment_description LIKE '%Settled%' OR payment_description LIKE '%Prepaid%' OR payment_description LIKE '%Past Due%'  OR payment_description LIKE '%Past Monthly%')  AND payment_description NOT LIKE '%Fee%'");
    ...
    if ($count <= 0 AND $billingDateSecs > $signUpSecs){
        ...
        $this->dueDate = date('Y-m-d',mktime(0,0,0,$i,25+6,2015));
    }elseif($count > 0){
        $overCount = $count-1;
     ...
     $stmt = $dbMain ->prepare("SELECT count(*), payment_date FROM payment_history WHERE payment_flag ='PF' AND contract_key='$this->contractKey' AND MONTH (payment_date) = '$i' AND YEAR (payment_date) = '2015' AND (payment_description LIKE '%New Service%')");
    $stmt->execute();      
    $stmt->store_result();      
...
$rgFeeArray = array('1','7');   

$this->numberRGMonthsOwed = 0;
   //echo "$i";$this->signup
$signup_dateSecs = strtotime($this->signup);
$rateStartSecs = strtotime('01/01/2012');
$rateJanSecs = strtotime('01/15/2015'); 
$rateMarchSecs = strtotime('03/15/2014'); 
$rateJulySecs = strtotime('07/15/2015'); 
        //echo "$signup_dateSecs $rateStartSecs";
$rateWEIRDStartSecs = strtotime('01/01/2014');
$rateWEIRDSecsEsd = strtotime('03/31/2014');
if($signup_dateSecs >= $rateStartSecs){
    if($signup_dateSecs <= $rateWEIRDSecsEsd AND $signup_dateSecs >= $rateWEIRDStartSecs){
        if($signup_dateSecs < $rateMarchSecs){
            $stmt = $dbMain ->prepare("SELECT count(*) FROM payment_history WHERE payment_flag ='PF' AND contract_key='$this->contractKey' AND YEAR (payment_date) = '2015' AND (payment_description = 'Guarantee Fee CC' OR payment_description = 'Guarantee Fee ACH' OR payment_description = 'Guarantee Fee Credit' OR payment_description = 'Guarentee Fee ACH' OR payment_description = 'Guarentee Fee CC' OR payment_description = 'Guarentee Fee Credit' OR payment_description = 'Rate Guarentee CC')");
            $stmt->execute();      
            $stmt->store_result();      
            $stmt->bind_result($count);
            $stmt->fetch();
          ...
          //  echo "ckey $this->contractKey month $i count $count  RATE FEE March<br>";
            $count = "";
        }
            ...
            $stmt->close();
             //echo "testvsvsd";
            if ($count <= 0 AND $signup_dateSecs < $rateJanSecs){
                $this->numberRGMonthsOwed = 2;
            }elseif($count <= 0 AND $signup_dateSecs > $rateJanSecs AND $signup_dateSecs < $rateJulySecs){
                $this->numberRGMonthsOwed = 1;
            }elseif($count <= 0 AND $signup_dateSecs > $rateJulySecs){
                $this->numberRGMonthsOwed = 0;
            }elseif ($count == 1 AND $signup_dateSecs < $rateJanSecs){
                $this->numberRGMonthsOwed = 1;
            }elseif($count == 1 AND $signup_dateSecs > $rateJanSecs AND $signup_dateSecs < $rateJulySecs){
                $this->numberRGMonthsOwed = 0;
            }elseif($count == 1 AND $signup_dateSecs > $rateJulySecs){
                $this->numberRGMonthsOwed = 0;
            }elseif ($count >= 2){
                $this->numberRGMonthsOwed = 0;
            }
                        
           //  echo "ckey $this->contractKey  count $count  RATE FEE jAN jUL<br>";
            $count = "";
