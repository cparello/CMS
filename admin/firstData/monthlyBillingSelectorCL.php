...
function dbconnect()   {
require"/var/www/vhosts/ems/cmp.burbankathleticclub.com/admin/dbConnect.php";//../dbConnect.php
return $dbMain;
}
                     ...
                     //   echo " $this->prePayCount ";
                      ...
                      //  echo " $this->monthlySettledCount $this->dueDate$dateFlag<br>";                                                                                                
                       ...
                       //     echo " $this->creditCount <br>";
                            ...
                            $batchId = "";                          
                             //echo "test;";
                            $stmt = $dbMain ->prepare("SELECT count(*) as count FROM billing_scheduled_recuring_payments WHERE contract_key ='$this->contractKey' AND payment_type='$paymentType' AND cycle_start_month = '$mStart' AND cycle_start_day = '$customerBillingDate' AND cycle_start_year = '$yearStart'");
                            $stmt->execute();      
                            $stmt->store_result();      
                            $stmt->bind_result($count);
                            $stmt->fetch();
                            ...
                            if ($count == 0 AND $this->billingAmount > 0){
                                ...
                                $stmt->bind_param('iiiissssssisssdssssssss',$pid, $batchId, $club_id, $this->contractKey, $processed, $imported, $outstandingBalance, $paymentType, $transactionType, $authID, $attemptNumber, $mStart, $customerBillingDate, $yearStart, $this->billingAmount, $responseMessage, $response, $response2, $avsResponse, $responseType, $exactReponse, $exactCode, $tranasctionTag );
                                if(!$stmt->execute())  {
...
include"/var/www/vhosts/ems/cmp.burbankathleticclub.com/admin/firstData/rateGuarenteeSelectorCL.php";
$upload = new rateFeeSelector();
$upload->setDay($day);
$upload->countRecord();

include"/var/www/vhosts/ems/cmp.burbankathleticclub.com/admin/firstData/enhanceFeeSelectorCL.php";
$upload = new enhanceFeeSelector();
$upload->setDay($day);
$upload->countRecord();

include"/var/www/vhosts/ems/cmp.burbankathleticclub.com/admin/firstData/maintnenceFeeSelectorCL.php";
$upload = new maintnenceFeeSelector();
$upload->setDay($day);
$upload->countRecord();

include"processScheduledCCTransactionsCL.php";
