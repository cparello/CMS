...
//echo "test";
$stmt99 = $dbMain ->prepare("SELECT payment_id, contract_key, payment_type, attempt_number, billing_amount, cycle_start_day, cycle_start_month, cycle_start_year, transaction_type FROM billing_scheduled_recuring_payments WHERE contract_key != '' AND processed = 'N' AND club_id = '3551' AND payment_type = 'PD'");
$stmt99->execute();      
$stmt99->store_result();      
...
while($stmt99->fetch()){
       ...
       if($transaction_type == 'CC'){
            ...
            if($expDateSecs >= $todayDateTestSecs){
                ...
                $expBool = 0;
                    ...
                    $trxnProperties = array(
                      ...
                      "PAN"=>""							//Used for debit transactions only		//Used for debit transactions only
                      );
        ...
        //var_dump($request);
            ...
            if($expBool == 0){
                ...
                $amountBilled = $trxnResult->DollarAmount;
                $attempt_number++;
            }elseif($expBool == 3){
                //DO NOTHING
                $attempt_number++;
    ...
    if ($amountBilled != $billing_amount){
        $billing_amount = $amountBilled;
        $currentBalance = $billing_amount - $amountBilled;
        $paymentFlag = "BD";
    }else{
        $currentBalance = 0;
        $paymentFlag = "PF";
    ...
    $achRequestID = 0;
        ...
        $achRequestID = $authId;
            ...
            $balanceDueDate = date("Y-m-d H:i:s");
            //$paymentFlag = 'PF';
            $rejectFeeCheck = 0;
            $rejectFeeCredit = 0;
            $rejectFeeAch = 0;
            $lateFeeAll = 0;
            $paymentDescription = "Past Monthly Dues $transaction_type";
            $historyKey = "";
            $contractKey = $contract_key;
            $paymentAmount = $billing_amount;                                            
            //$paymentDate = $this->paymentDate;   
            $paymentDate = date('Y-m-d H:i:s',mktime(0,0,0,$cycle_start_month, $cycle_start_day, $cycle_start_year)); //$this->paymentDate;                     
            $transKey = $authId;                                          
            $checkNumber = '0';
            $bundled = 'N';
            
            $stmt = $dbMain ->prepare("SELECT count(*) FROM payment_history WHERE contract_key = '$contract_key' AND MONTH(payment_date) = '$cycle_start_month' AND YEAR(payment_date) = '$cycle_start_year' AND payment_description = '$paymentDescription' AND payment_flag = 'PF'");
            $stmt->execute();      
            $stmt->store_result();      
            ...
            if($count == 0){
                ...
                $imported = 'Y';
              $sql = "UPDATE billing_scheduled_recuring_payments SET imported = ? WHERE contract_key = '$contract_key' AND payment_type = 'PD'";
              $stmt = $dbMain->prepare($sql);
...
$stmt99 = $dbMain ->prepare("SELECT payment_id, contract_key, payment_type, attempt_number, billing_amount, cycle_start_day, cycle_start_month, cycle_start_year, transaction_type FROM billing_scheduled_recuring_payments WHERE contract_key != '' AND processed = 'N' AND club_id = '3552' AND payment_type = 'PD'");
$stmt99->execute();      
$stmt99->store_result();      
$stmt99->bind_result($payment_id, $contract_key, $payment_type, $attempt_number, $billing_amount, $cycle_start_day, $cycle_start_month, $cycle_start_year, $transaction_type);
while($stmt99->fetch()){
...
$update->setLesserValue($collectAmount);
$update->moveData();
