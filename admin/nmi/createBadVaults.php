...
$stmt33 = $dbMain ->prepare("SELECT DISTINCT contract_key, transaction_type FROM batch_recurring_records WHERE vault_id = '0'");
$stmt33->execute();      
$stmt33->store_result();      
$stmt33->bind_result($contractKey, $monthly_billing_type);
while($stmt33->fetch()){
            ...
            if($monthly_billing_type == 'CC'){
                ...
                if($ccAuthReasonCode == 100){
                    ...
                    $no = 'N';
                    $attempt = 1;
                    $sql = "UPDATE batch_recurring_records SET vault_id = ?, record_batched = ?, processed = ?, imported = ?, outstanding_balance = ?, attempt_number = ?  WHERE contract_key = '$contractKey'";
                    $stmt = $dbMain->prepare($sql);
                    $stmt->bind_param('sssssi', $contractKey, $no, $no, $no, $no, $attempt);
                    $stmt->execute();                    
                    $stmt->close();
                }else{
                    ...
                    $stmt = $dbMain->prepare($sql);
                    $stmt->bind_param('idis', $contractKey, $owed, $monthsPast, $dateIn); 
                    $stmt->execute();
                    $stmt->close(); 
        ...
        echo "contract key  $contractKey bill type $monthly_billing_type decision $ccAuthDecision auth $authCode  <br> <br>";   
