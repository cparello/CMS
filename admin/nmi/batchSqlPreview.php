...
$stmt1 = $dbMain->prepare("SELECT COUNT(*) as count FROM member_maintnence_eft WHERE contract_key!=''");
$stmt1->execute();      
$stmt1->store_result();      
$stmt1->bind_result($rowCount); 
$stmt1->fetch();
$stmt1->close();
...
foreach($clubArray as $clubId){
        ...
        $stmt = $dbMain ->prepare("SELECT COUNT(*) FROM billing_scheduled_preview WHERE cycle_start_month= '$mStart' AND cycle_start_day = '$billingDate' AND  cycle_start_year = '$yStart' AND payment_type = 'MF' AND club_id = '$clubId'");
            ...
            if ($totalBillingAmountProjected > 0){
                ...
                if ($count >= 1){
                    $sql = "UPDATE billing_monthly_batch_previews SET projected = ?, credit_total_projected = ?, ach_total_projected = ?  , number_records = ? WHERE month= '$mStart' AND day = '$billingDate' AND  year = '$yStart' AND batch_type = 'MF' AND club_id = '$clubId'";
                  $stmt = $dbMain->prepare($sql);
                  $stmt->bind_param('dddi', $totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch, $count);
                  if(!$stmt->execute())  {
                  ...
                  $stmt->close(); 
                	...
                	$stmt = $dbMain->prepare($sql);
                	$stmt->bind_param('isiiiidddi',$bid, $batch_type, $clubId, $mStart, $billingDate, $yStart, $totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch,$count);
                	if(!$stmt->execute())  {
...
$stmt1 = $dbMain->prepare("SELECT COUNT(*) as count FROM member_enhance_eft WHERE contract_key!=''");
$stmt1->execute();      
$stmt1->store_result();      
$stmt1->bind_result($rowCount); 
$stmt1->fetch();
        ...
        $stmt = $dbMain ->prepare("SELECT COUNT(*) FROM billing_scheduled_preview WHERE cycle_start_month= '$mStart' AND cycle_start_day = '$billingDate' AND  cycle_start_year = '$yStart' AND payment_type = 'EF' AND club_id = '$clubId'");
            ...
            if ($totalBillingAmountProjected > 0){
               ...
               // $percentage = $totalBillingSucess/$totalBillingAmountProjected;
                ...
                if ($count >= 1){
                    $sql = "UPDATE billing_monthly_batch_previews SET projected = ?, credit_total_projected = ?, ach_total_projected = ?  , number_records = ? WHERE month= '$mStart' AND day = '$billingDate' AND  year = '$yStart' AND batch_type = 'EF' AND club_id = '$clubId'";
                  $stmt = $dbMain->prepare($sql);
                  $stmt->bind_param('dddi', $totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch, $count);
                  if(!$stmt->execute())  {
...
$stmt1 = $dbMain->prepare("SELECT COUNT(*) as count FROM member_guarantee_eft WHERE contract_key!=''");
$stmt1->execute();      
$stmt1->store_result();      
$stmt1->bind_result($rowCount); 
$stmt1->fetch();
  ...
  foreach($clubArray as $clubId){
         ...
         $stmt = $dbMain ->prepare("SELECT COUNT(*) FROM billing_scheduled_preview WHERE cycle_start_month= '$mStart' AND cycle_start_day = '$billingDate' AND  cycle_start_year = '$yStart' AND payment_type = 'RF' AND club_id = '$clubId'");
            ...
            if ($totalBillingAmountProjected > 0){
                ...
                if ($count >= 1){
                     $sql = "UPDATE billing_monthly_batch_previews SET projected = ?, credit_total_projected = ?, ach_total_projected = ?  , number_records = ? WHERE month= '$mStart' AND day = '$billingDate' AND  year = '$yStart' AND batch_type = 'RF' AND club_id = '$clubId'";
                  $stmt = $dbMain->prepare($sql);
                  $stmt->bind_param('dddi', $totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch, $count);
                  if(!$stmt->execute())  {
                 	  printf("Error:updateEHFEE %s.\n", $stmt->error);
              ...
              //  $percentage = $totalBillingSucess/$totalBillingAmountProjected;
                ...
                if ($count >= 1){
                   ...
                   //echo "$totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch, $totalBillingSucess, $totalBillingSucessCc, $totalBillingSucessAch, $totalBillingFailed, $totalBillingFailedCc, $totalBillingFailedAch, $percentage, $totalNumberAttempted, $totalNumberSuccess, $totalNumberFailed,  $mStart' AND day = '$billingDate' AND  year = '$yStart";                   
                     ...
                     $sql = "UPDATE billing_monthly_batch_previews SET projected = ?, credit_total_projected = ?, ach_total_projected = ? , number_records = ? WHERE month= '$mStart' AND day = '$billingDate' AND  year = '$yStart' AND batch_type = 'MS' AND club_id = '$clubId'";
                  $stmt = $dbMain->prepare($sql);
                  // echo "count $count ";
                  $stmt->bind_param('dddi', $totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch, $count);
          ...
          $stmt = $dbMain ->prepare("SELECT COUNT(*) FROM billing_scheduled_preview WHERE cycle_start_month= '$mStart'  AND  cycle_start_year = '$yStart' AND payment_type = 'PD' AND club_id = '$clubId'");
            ...
            if ($totalBillingAmountProjected > 0){
                ...
                if ($count >= 1){
                   ...
                   //echo "$totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch, $totalBillingSucess, $totalBillingSucessCc, $totalBillingSucessAch, $totalBillingFailed, $totalBillingFailedCc, $totalBillingFailedAch, $percentage, $totalNumberAttempted, $totalNumberSuccess, $totalNumberFailed,  $mStart' AND day = '$billingDate' AND  year = '$yStart";                   
                    //echo "test";
                     $sql = "UPDATE billing_monthly_batch_previews SET projected = ?, credit_total_projected = ?, ach_total_projected = ?, number_records = ?  WHERE month= '$mStart' AND  year = '$yStart' AND batch_type = 'PD' AND club_id = '$clubId'";
                  $stmt = $dbMain->prepare($sql);
                  // echo "count $count ";
                  $stmt->bind_param('dddi', $totalBillingAmountProjected, $totalBillingAmountProjectedCc, $totalBillingAmountProjectedAch, $count);
